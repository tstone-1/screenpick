...
// A persisted annotation layer as it sits in `annotations.json`: the layer
// itself plus the identity of the base raster it was drawn against.
//
// `baseFile` is the file NAME of the document's base raster (`base.png`, or
// `base-<ms>-<seq>.png` after a re-base), never a full path. Rust's manifest
// stores exactly that name (`DocumentMeta::base_file` in document_store.rs) and
// derives every record's `basePath` from it, so the name is the one identity
// both ends can compare byte-for-byte: a full path carries the platform's
// separator and the user's app-data directory, neither of which is stable
// across machines or a moved data folder, while the name is ASCII generated by
// Rust and never rewritten.
//
// `null` means an un-stamped layer — every document written before the stamp
// existed, plus anything saved for a document whose base file the store does
// not know. Such a layer is applied as-is; see `annotationLayerForBase`.
export type StoredAnnotationLayer = {
  annotations: Annotation[];
  baseFile: string | null;
};

// Serialize an annotation layer for persistence (`annotations.json`), stamped
// with the base raster it was rendered against when the caller knows it.
//
// The stamp is what makes the crop/cut save survivable. Re-basing
// (`replace_document_base`) and writing the layer (`save_document`) are two
// separate IPC calls, and the first one commits the new base to the manifest —
// so a crash, a full disk, or a forced quit between them leaves the cropped
// base beside the pre-crop layer. Restoring that pair re-applies every
// annotation over an image it was never drawn on, offset by the crop origin,
// silently and permanently. A stamped layer lets the restore path notice the
// pairing is wrong and drop the layer instead of misplacing it.
//
...
export function serializeAnnotations(
  ...
  if (baseFile === null) return JSON.stringify(annotations);
  return JSON.stringify({ baseFile, annotations });
}

// Resolve a restored layer against the base raster the document actually holds
// now. A stamp naming a different base file means the two halves of a crop/cut
// save did not both land: the layer belongs to the pre-crop image, and applying
// it would offset every annotation. Dropping it keeps the base image and loses
// the overlay — recoverable-looking damage the user can see, rather than
// annotations sitting confidently in the wrong places.
//
// `droppedFrom` carries the stamp that was refused so the caller can name both
// identities in the diagnostics log; `null` means the layer was applied.
export function annotationLayerForBase(
  stored: StoredAnnotationLayer,
  baseFile: string
): { annotations: Annotation[]; droppedFrom: string | null } {
  if (stored.baseFile !== null && stored.baseFile !== baseFile) {
    return { annotations: [], droppedFrom: stored.baseFile };
  }
  return { annotations: stored.annotations, droppedFrom: null };
}
...
export function deserializeAnnotations(json: string): Annotation[] {
  return deserializeAnnotationLayer(json).annotations;
}

// The full persisted layer, stamp included (see `StoredAnnotationLayer`). Both
// on-disk shapes are accepted, and must stay accepted: the bare array every
// existing document holds, and the stamped envelope written since. An upgrade
// must not turn a legacy layer into a mismatch and drop the user's work, so a
// missing stamp reads as "unknown", never as "does not match".
export function deserializeAnnotationLayer(json: string): StoredAnnotationLayer {
  let parsed: unknown;
  try {
    parsed = JSON.parse(json);
  } catch {
    return { annotations: [], baseFile: null };
  }
  if (Array.isArray(parsed)) {
    return { annotations: validAnnotationsIn(parsed), baseFile: null };
  }
  if (typeof parsed === "object" && parsed !== null) {
    const envelope = parsed as Record<string, unknown>;
    if (Array.isArray(envelope.annotations)) {
      return {
        annotations: validAnnotationsIn(envelope.annotations),
        // A non-string stamp is read as no stamp. The layer is data we do not
        // control, and of the two readings of a damaged field, applying the
        // layer is the one that matches what an un-stamped layer already does;
        // dropping work over a malformed key would be the harsher call.
        baseFile: typeof envelope.baseFile === "string" ? envelope.baseFile : null
      };
    }
  }
  return { annotations: [], baseFile: null };
...
function validAnnotationsIn(entries: unknown[]): Annotation[] {
  return entries.filter((entry): entry is Annotation => {
